...
def save_new_data(tracks):
    ...
    for item in tracks['items']:
        track = item['track']
        artist = ", ".join(art['name'] for art in track['artists']) if len(track['artists']) > 1 else track['artists'][0]['name']
        name = track['name'].strip()
        cursor.execute('SELECT EXISTS(SELECT 1 FROM main WHERE "Song Name" = ? and "Artists" = ?)', (name, artist))
        #print("space" if name[-1] == ' ' else name)
        ...
        if exists:
            ...
            new_row = {"Song Name": [name], "Artists": [artist], "Album": [track['album']['name']],
                       "Image_Info": str([track['album']['images'][0]]), "Times Played": [1]}
            db = pd.DataFrame(new_row)
            #db2 = pd.read_sql("SELECT * FROM main")
            #print(db2.columns)
            #print(type(item['played_at']))
            db.to_sql("main", conn, if_exists="append", index=False)
            new_row = {"Song": [name], "Artist": [artist], "Listen History": [item['played_at']]}
            db = pd.DataFrame(new_row)
            db.to_sql("times", conn, if_exists="append", index=False)
        #ARTIST UPDATE
        ...
        for a in art:
            #print(a)
            a = a.strip()
            cursor.execute('SELECT EXISTS(SELECT 1 FROM artists WHERE "Artist" = ?)', (a,))
            exists = cursor.fetchone()[0]
            print("artists ", exists)
            if exists:
                cursor.execute('SELECT "Songs Listened To" FROM artists WHERE "Artist" = ?', (a,))
                num_a = cursor.fetchone()[0]
               # n_a = num_a[1] + 1
                cursor.execute('UPDATE artists SET "Songs Listened To" = ? WHERE "Artist" = ?', (num_a+1, a))
            else:
                new_row = {"Artist": [a], "Songs Listened To": [1]}
                db = pd.DataFrame(new_row)
                db.to_sql("artists", conn, if_exists="append", index=False)
        ...
        print(alb_art=="The Weeknd")
        # print(alb)
        cursor.execute('SELECT EXISTS (SELECT 1 FROM albums WHERE "Album" = ? AND "Artist" = ?)', (alb, alb_art))
        exists = cursor.fetchone()[0]
        print("album ", exists)
        if exists:
            print("a")
            cursor.execute('SELECT "Number of Songs" FROM albums WHERE "Album" = ? AND "Artist" = ?', (alb, alb_art))
            num_alb = cursor.fetchone()
            cursor.execute('UPDATE albums SET "Number of Songs" = ? WHERE "Album" = ? AND "Artist" = ?', (num_alb[0]+1, alb, alb_art))
        else:
            new_row = {"Album": [alb],  "Artist": [alb_art], "Number of Songs": [1]}
            db = pd.DataFrame(new_row)
            db.to_sql("albums", conn, if_exists="append", index=False)
...
def sort_dbs():
    ...
    cursor.execute('SELECT * FROM main ORDER BY "Times Played" DESC')
    cursor.execute('SELECT * FROM albums ORDER BY "Number of Songs" DESC')
    cursor.execute('SELECT * FROM artists ORDER BY "Songs Listened To" DESC')
